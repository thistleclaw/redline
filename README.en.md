...
- `:test ai model <scenario>` — ask Gemini for bounded input assumptions, calculate an extended
  spatial SEIR model locally with NumPy, then ask Gemini to explain the bounded local result in
  plain language without changing its numbers. A Braille spinner stays active throughout the
  pipeline. `duration=` and `speed=` work as for timelapse; calculated rows include Rt, active
  infections, hospital load, deaths and a deterministic P10–P90 ensemble range. See the
  [model notes](docs/SEIR_MODEL.md).
