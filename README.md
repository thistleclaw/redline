...
:test ai model [duration=1y] [speed=1w/1s] <сценарий>
:test pause|play|step
:test off
```

`:forecast ai` — одна команда; последний аргумент выбирает ветвь: `bad` строит реалистичную
неблагоприятную траекторию, `good` — благоприятную. Это условные AI-сценарии на горизонтах 2–4 недель и
...
`:test ai model` — гибридный режим: Gemini выбирает входные гипотезы и географические узлы,
после строгой проверки весь временной ряд рассчитывает локальный NumPy-движок, а затем Gemini
переводит ограниченный результат расчёта на понятный язык, не меняя чисел. Во время каждого
этапа Инспектор показывает анимированный Braille-индикатор. Локальный движок расширяет
SEIR бессимптомной передачей, госпитализацией, смертностью, вакцинацией, ослаблением иммунитета,
сезонностью, вмешательствами, импортированными случаями и пространственной мобильностью. На карте
проигрывается локально вычисленный таймлапс, а инспектор показывает Rt, активные инфекции,
госпитализации, смерти и воспроизводимый интервал P10–P90. Подробнее: [модель и допущения](docs/SEIR_MODEL.md).
...
AI-команды передают Gemini нормализованный контекст монитора или ограниченный синтетический
результат локальной SEIR-модели, но не SQLite-файл, полный кэш документов, exports, локальные
пути или audit log. Подробная модель доверия описана в
